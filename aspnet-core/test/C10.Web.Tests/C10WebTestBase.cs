...
public abstract class C10WebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
{
    protected static readonly Lazy<string> ContentRootFolder;

    static C10WebTestBase()
    {
    ...
    protected override IWebHostBuilder CreateWebHostBuilder()
    {
        return base
            ...
            .UseSetting(WebHostDefaults.ApplicationKey, typeof(C10WebMvcModule).Assembly.FullName);
    }
    ...
    {
        using (var context = IocManager.Resolve<C10DbContext>())
        {
            action(context);
            context.SaveChanges();
        }
    ...
    protected async Task UsingDbContextAsync(Func<C10DbContext, Task> action)
    {
        using (var context = IocManager.Resolve<C10DbContext>())
        {
    ...
    protected async Task<T> UsingDbContextAsync<T>(Func<C10DbContext, Task<T>> func)
    {
        T result;
